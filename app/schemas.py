...
class UserCreate(BaseModel):
    ...
    user_email: Optional[EmailStr] = None  # 이메일 (일반 회원가입용)
    user_password: Optional[str] = None  # 비밀번호 (일반 회원가입용)
    user_gender: Optional[str] = None  # 성별 (선택사항)
    user_age: Optional[int] = None  # 나이 (선택사항)
    user_region: Optional[str] = None  # 거주 지역 (선택사항)
    user_kakao_id: Optional[str] = None  # 카카오 고유 ID (카카오 로그인용)
...
class KakaoRegisterRequest(BaseModel):
    user_kakao_id: str  # 카카오 고유 ID (필수)
...
class UserResponse(BaseModel):
    user_id: int  # PK (필수)
    user_kakao_id: Optional[str] = None  # 카카오 ID (선택)
